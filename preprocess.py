...
def build_save_text_dataset_in_shards(src_corpus, tgt_corpus, ctx_corpus, fields,
                                      corpus_type, opt):
    '''
    ...
    print("origin corpus size: {}".format(corpus_size))
    ...
    src_back_iter = onmt.io.ShardedTextCorpusIterator(
                tgt_corpus, opt.src_seq_length_trunc,
                "src_back", opt.max_shard_size, assoc_iter=src_iter)

    ctx_iter = onmt.io.ShardedTextCorpusIterator(
                ctx_corpus, opt.ctx_seq_length_trunc,
                "ctx", opt.max_shard_size, assoc_iter=src_iter)

    tgt_iter = onmt.io.ShardedTextCorpusIterator(
                tgt_corpus, opt.tgt_seq_length_trunc,
                "tgt", opt.max_shard_size,
                assoc_iter=src_iter)
    tgt_back_iter = onmt.io.ShardedTextCorpusIterator(
                src_corpus, opt.tgt_seq_length_trunc,
                "tgt_back", opt.max_shard_size,
                assoc_iter=src_iter)
    ...
    while not src_iter.hit_end():
        ...
        dataset = onmt.io.TextDataset(
                fields, src_iter, tgt_iter, ctx_iter, src_back_iter, tgt_back_iter,
                src_iter.num_feats, tgt_iter.num_feats, ctx_iter.num_feats,
                src_seq_length=opt.src_seq_length,
                ctx_seq_length=opt.ctx_seq_length,
...
def build_save_dataset(corpus_type, fields, opt):
    ...
    if opt.data_type == 'text':
        return build_save_text_dataset_in_shards(
                src_corpus, tgt_corpus, ctx_corpus, fields,
                corpus_type, opt)
...
def main():
    ...
    src_back_nfeats = onmt.io.get_num_features(opt.data_type, opt.train_tgt, 'src_back')
    tgt_nfeats = onmt.io.get_num_features(opt.data_type, opt.train_tgt, 'tgt')
    tgt_back_nfeats = onmt.io.get_num_features(opt.data_type, opt.train_src, 'tgt_back')
    print(" * number of source features: %d." % src_nfeats)
    print(" * number of ctx features: %d." % ctx_nfeats)
