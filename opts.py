...
def model_opts(parser):
    ...
    group.add_argument('-mirror_type', default='mirror',
                       help="""Options are [mirror, mirror_light].""")
    ...
    group.add_argument('-z_dim', type=int, default=100,
                       help='dim num of z')
    ...
    group.add_argument('-input_feed_with_ctx', type=int, default=1,
                       help="""Feed the context vector at each time step as
                       additional input (via concatenation with the word
                       embeddings) to the decoder.""")
    ...
    group.add_argument('-ctx_bid', action="store_true", help='ctx encoder is bidirectional.')
    group.add_argument('-ctx_cat', action="store_true", help='concatenate the context from c and x, or c and y (for attention calculation)')
    ...
    group.add_argument('-cz_no_c', action="store_true",
                       help=' remove c for cz2xy')
    # Attention options
...
def preprocess_opts(parser):
    ...
    group.add_argument('-train_ctx', required=True,
                       help="Path to the training ctx data")
    group.add_argument('-train_tgt', required=True,
                       help="Path to the training target data")
    group.add_argument('-valid_src', required=True,
                       help="Path to the validation source data")
    group.add_argument('-valid_ctx', required=True,
                       help="Path to the validation ctx data")
    ...
    group.add_argument('-ctx_seq_length', type=int, default=100,
                       help="Maximum source sequence length")
...
def train_opts(parser):
    ...
    # group.add_argument('-kl_balance', action='store_true', help='--')
    group.add_argument('-kl_balance', type=float, default=0.0001,
                       help="from which epoch to set kl_knealling=1.0; the default value 0.00001 means it starts from epoch 0")
    ...
    group.add_argument('-kl_fix', type=float, default=-1,
                       help="the kl_factor value for mirror_light model; -1 means the value is not fixed and will increase like the mirror_full setup.")
    ...
    group.add_argument('-window_size', type=float, default=.02,
                       help="Window size for spectrogram in seconds.")
    group.add_argument('-back_factor', type=float, default=1.0,
                       help="the factor to control the backforward contribution.")
...
def translate_opts(parser):
    ...
    group.add_argument('-mirror_type', default='mirror',
                       help="""Options are [mirror, mirror_low].""")
    # MMI optional model
    ...
    group.add_argument('-src',   required=True,
                       ...
                       sequence)""")
    group.add_argument('-ctx',   required=True,
                       help="""Source sequence to decode (one line per
                       sequence)""")
    group.add_argument('-src_dir',   default="",
                       help='Source directory for image or audio files')
    ...
    group.add_argument('-beam_size',  type=int, default=5,
                       help='Beam size')
    group.add_argument('-use_dc',  type=int, default=-1,
                       help='Beam size')
    group.add_argument('-min_length', type=int, default=1,
                       help='Minimum prediction length')
    group.add_argument('-max_length', type=int, default=40,
