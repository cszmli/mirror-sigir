...
class MirrorCopyGeneratorLossCompute(onmt.Loss.LossComputeBase):
    """
    Copy Generator Loss Computation.
    """
    def __init__(self, generator, tgt_vocab,
                 force_copy, eps=1e-20):
        super(MirrorCopyGeneratorLossCompute, self).__init__(
            generator, tgt_vocab)
    ...
    def _make_shard_state(self, batch, output, range_, attns, back):
        """ See base class for args description. """
        if getattr(batch, "alignment", None) is None:
            raise AssertionError("using -copy_attn you need to pass in "
                                 "-dynamic_dict during preprocess stage.")
        if not back:
            ...
            return {
                ...
                "copy_attn": attns.get("copy"),
                "align": batch.alignment_back[range_[0] + 1: range_[1]]
            }
            

    def _compute_loss(self, back, ctx_src, batch, output, target, copy_attn, align):
        """
        Compute the loss. The args must match self._make_shard_state().
        Args:
            batch: the current batch.
        ...
        target = target.view(-1)
        align = align.view(-1)
        map_ = batch.src_map if not back else batch.src_back_map
        # if it's cz2x or cz2y, we need to use ctx_map to override map_
        map_ = batch.ctx_map if ctx_src else map_
        scores = self.generator(self._bottle(output),
                                self._bottle(copy_attn),
                                map_)
