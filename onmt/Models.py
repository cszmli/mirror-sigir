...
class EncoderStateFix(nn.Module):
    ...
    def __init__(self,opt):
        ...
        if opt.rnn_type=='LSTM':
            self.encoder2z_layer1_h = nn.Sequential(
                nn.Linear(opt.rnn_size * enc_layers, int(opt.rnn_size/2)),
                nn.ReLU()
            )
    ...
    def forward(self, encoder_hidden):
        ...
        if isinstance(encoder_hidden, tuple):
            ...
            h1 = encoder_hidden[0]
            c1 = encoder_hidden[1]
            # logging.info(h1.shape)
            # logging.info(c1.shape)
            h1 = self.encoder2z_layer1_h(h1.transpose(0,1).contiguous().view(batch_size, -1))
            c1 = self.encoder2z_layer1_c(c1.transpose(0,1).contiguous().view(batch_size, -1))
...
class BuildDecoderState(nn.Module):
    ...
    def __init__(self,opt):
        ...
        if opt.rnn_type=='GRU':
            ...
            self.cxz2y_h = nn.Sequential(
                    nn.Linear(enc_size * 2, decoder_size),
                    nn.ReLU()
                )
            ...
            self.cz2xy_h = nn.Sequential(
                        nn.Linear(enc_size, decoder_size),
                        nn.ReLU()
                    )
    ...
    def forward(self, cxz, cyz, cz):
        if self.rnn_type=='GRU':
            ...
            cxz_h = cxz_h.view(-1, self.num_layer, self.rnn_size).transpose(0,1)
            cyz_h = cyz_h.view(-1, self.num_layer, self.rnn_size).transpose(0,1)
            cz_h = cz_h.view(-1, self.num_layer, self.rnn_size).transpose(0,1)
            cxz_c = cxz_c.view(-1, self.num_layer, self.rnn_size).transpose(0,1)
            cyz_c = cyz_c.view(-1, self.num_layer, self.rnn_size).transpose(0,1)
            cz_c = cz_c.view(-1, self.num_layer, self.rnn_size).transpose(0,1)
...
class MirrorLight(MirrorModel):
    def __init__(self, encoder_utt, encoder_utt_y, encoder_ctx, decoder_cxz2y, decoder_cz2x, decoder_cyz2x, decoder_cz2y,multigpu=False, opt=None):
        ...
        self.encoder2z = EncoderState2Z_light(opt)
    ...
    def forward(self, src, tgt, ctx, lengths, src_back, tgt_back, lengths_back, lengths_ctx, dec_state_cxz2y=None, dec_state_cyz2x=None, dec_state_cz2x=None, dec_state_cz2y=None, ctx_cat=None):
        ...
        rec_z, rec_mu, rec_logvar = self.encoder2z.cx2z(enc_hidden_cx)  # this is the light version: p(z|c,x) is the posterior estimation
        real_z = rec_z
...
class EncoderState2Z_light(nn.Module):
    """
    """
    def __init__(self,opt):
        super(EncoderState2Z_light, self).__init__()
        enc_size = opt.rnn_size 
        z_dim = opt.z_dim
        # Prior Net
...
class MirrorLow(MirrorModel):
    def __init__(self, encoder_utt, encoder_utt_y, encoder_ctx, decoder_cxz2y, decoder_cz2x, decoder_cyz2x, decoder_cz2y,multigpu=False, opt=None):
        super(MirrorLow, self).__init__(encoder_utt, encoder_utt_y, encoder_ctx, decoder_cxz2y, decoder_cz2x, decoder_cyz2x, decoder_cz2y,multigpu=multigpu, opt=opt)
        self.encoder2z = EncoderState2Z_low(opt)
    ...
    def forward(self, src, tgt, ctx, lengths, src_back, tgt_back, lengths_back, lengths_ctx, dec_state_cxz2y=None, dec_state_cyz2x=None, dec_state_cz2x=None, dec_state_cz2y=None, ctx_cat=None):
        ...
        enc_hidden_cx, enc_hidden_cy, enc_hidden_c = self.decoder_init_builder(enc_hidden_cx, enc_hidden_cy, enc_hidden_c)

        if not self.ctx_cat:
            enc_state_cx = RNNDecoderState(context_x, self.hidden_size, enc_hidden_cx)
            enc_state_cy = RNNDecoderState(context_y, self.hidden_size, enc_hidden_cy)
            enc_state_c = RNNDecoderState(context_ctx, self.hidden_size, enc_hidden_c)
            out_cxz2y, dec_state_cxz2y, attns_cxz2y = self.decoder_cxz2y(real_z, tgt, context_x,
                                            enc_state_cx if dec_state_cxz2y is None else dec_state_cxz2y,
                                            context_lengths=lengths)

            out_cyz2x, dec_state_cyz2x, attns_cyz2x = self.decoder_cyz2x(real_z,tgt_back, context_y,
                                            enc_state_cy if dec_state_cyz2x is None else dec_state_cyz2x,
                                            context_lengths=lengths_back)
            ...
            enc_state_cx = RNNDecoderState(context_ctx_x, self.hidden_size, enc_hidden_cx)
            enc_state_cy = RNNDecoderState(context_ctx_y, self.hidden_size, enc_hidden_cy)
            enc_state_c = RNNDecoderState(context_ctx, self.hidden_size, enc_hidden_c)

            out_cxz2y, dec_state_cxz2y, attns_cxz2y = self.decoder_cxz2y(real_z, tgt, context_ctx_x, enc_state_cx if dec_state_cxz2y is None else dec_state_cxz2y)

            out_cyz2x, dec_state_cyz2x, attns_cyz2x = self.decoder_cyz2x(real_z, tgt_back, context_ctx_y, enc_state_cy if dec_state_cyz2x is None else dec_state_cyz2x)
            ######################################################


        out_cz2x, dec_state_cz2x, attns_cz2x = self.decoder_cz2x(real_z,tgt_back, context_ctx,
                                        enc_state_c if dec_state_cz2x is None else dec_state_cz2x,
                                        context_lengths=lengths_ctx)

        out_cz2y, dec_state_cz2y, attns_cz2y = self.decoder_cz2y(real_z,tgt, context_ctx,
                                            enc_state_c if dec_state_cz2y is None else dec_state_cz2y,
                                            context_lengths=lengths_ctx)
        kl_loss = self.encoder2z.kl_loss(rec_mu, rec_logvar, prior_mu, prior_logvar)
        results = Pack(
                        out_cxz2y=out_cxz2y, dec_state_cxz2y=dec_state_cxz2y, attns_cxz2y=attns_cxz2y,
    ...
    def forward_beam(self, src, ctx, lengths, lengths_ctx, ctx_cat=None):
        ...
        prior_z, prior_mu, prior_logvar = self.encoder2z.cx2z(enc_hidden_cx)
        enc_hidden_cx = self.decoder_init_builder.forward_single(enc_hidden_cx)
        if self.ctx_cat:
            enc_state_cx = RNNDecoderState(context_ctx_x, self.hidden_size, enc_hidden_cx)
            return enc_state_cx, context_ctx_x, prior_z
        else:
            enc_state_cx = RNNDecoderState(context_x, self.hidden_size, enc_hidden_cx)
            return enc_state_cx, context_x, prior_z
...
class EncoderState2Z_low(nn.Module):
    """
    """
    def __init__(self,opt):
        super(EncoderState2Z_low, self).__init__()
        enc_size = opt.rnn_size 
        z_dim = opt.z_dim
        self.cxy2z_layer = nn.Sequential(
                nn.Linear(enc_size * 3, z_dim * 2),
