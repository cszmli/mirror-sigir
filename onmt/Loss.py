...
class LossComputeBase(nn.Module):
    ...
    def mirror_monolithic_compute_loss(self, batch, output, attns, back, ctx_src):
        """
        Compute the forward loss for the batch.
        ...
        if not back:
            ...
            range_ = (0, batch.tgt_back.size(0))
        shard_state = self._make_shard_state(batch, output, range_, attns, back)
        _, batch_stats = self._compute_loss(back, ctx_src, batch, **shard_state)
    ...
    def mirror_compute_loss(self, batch, output, attns,
                             cur_trunc=None, trunc_size=None, shard_size=None,
                             normalization=None, back=False, ctx_src=False):
        """Compute the forward loss and backpropagate.  Computation is done
        with shards and optionally truncation for memory efficiency.
        ...
        if not back:
            ...
            range_ = (0, batch.tgt_back.size(0))
        shard_state = self._make_shard_state(batch, output, range_, attns, back)
        loss, batch_stats = self._compute_loss(back, ctx_src, batch, **shard_state)
        return batch_stats, loss
    
    def mirror_compute_update_loss(self, batch, output, attns,
                             cur_trunc=None, trunc_size=None, shard_size=None,
                             normalization=None, back=False, ctx_src=False):
        """Compute the forward loss and backpropagate.  Computation is done
        with shards and optionally truncation for memory efficiency.
        ...
        for shard in shards(shard_state, shard_size, retain_graph=True):
            loss, stats = self._compute_loss(back, ctx_src, batch, **shard)
...
class MirrorNMTLossCompute(LossComputeBase):
    """
    Standard NMT Loss Computation.
    """
    def __init__(self, generator, tgt_vocab, normalization="sents",
                 label_smoothing=0.0):
        super(MirrorNMTLossCompute, self).__init__(generator, tgt_vocab)
        assert (label_smoothing >= 0.0 and label_smoothing <= 1.0)
    ...
    def _make_shard_state(self, batch, output, range_, attns=None, back=False):
        if not back:
            ...
            return {
                "output": output,
                "target": batch.tgt_back[range_[0] + 1: range_[1]],
            }

    def _compute_loss(self, back, ctx_src, batch, output, target):
        scores = self.generator(self._bottle(output))
