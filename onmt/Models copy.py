...
class MirrorModel(nn.Module):
    ...
    def __init__(self, encoder_utt, encoder_utt_y, encoder_ctx, decoder_cxz2y, decoder_cz2x, decoder_cyz2x, decoder_cz2y,multigpu=False, opt=None):
        ...
        self.encoder_state_fix_xy = EncoderStateFix(opt)
    ...
    def forward(self, src, tgt, ctx, lengths, src_back, tgt_back, lengths_back, lengths_ctx, dec_state_cxz2y=None, dec_state_cyz2x=None, dec_state_cz2x=None, dec_state_cz2y=None, ctx_cat=True):
        """Forward propagate a `src` and `tgt` pair for training.
        Possible initialized with a beginning decoder state.
        ...
        prior_z, prior_mu, prior_logvar = self.encoder2z.c2z(enc_hidden_ctx.detach())
        # (256, z_dim)
        ...
        if not ctx_cat:
            ...
            out_cxz2y, dec_state_cxz2y, attns_cxz2y = self.decoder_cxz2y(rec_z, tgt, context_x,
                                            enc_state_cxz if dec_state_cxz2y is None else dec_state_cxz2y,
                                            context_lengths=lengths)

            out_cyz2x, dec_state_cyz2x, attns_cyz2x = self.decoder_cyz2x(rec_z,tgt_back, context_y,
            ...
            out_cxz2y, dec_state_cxz2y, attns_cxz2y = self.decoder_cxz2y(rec_z, tgt, context_ctx_x, enc_state_cxz if dec_state_cxz2y is None else dec_state_cxz2y)

            out_cyz2x, dec_state_cyz2x, attns_cyz2x = self.decoder_cyz2x(rec_z, tgt_back, context_ctx_y,
                                            enc_state_cyz if dec_state_cyz2x is None else dec_state_cyz2x)
            ######################################################


        out_cz2x, dec_state_cz2x, attns_cz2x = self.decoder_cz2x(rec_z,tgt_back, context_ctx,
                                        enc_state_cz if dec_state_cz2x is None else dec_state_cz2x,
                                        context_lengths=lengths_ctx)

        out_cz2y, dec_state_cz2y, attns_cz2y = self.decoder_cz2y(rec_z,tgt, context_ctx,
    ...
    def forward_beam(self, src, ctx, lengths, lengths_ctx, ctx_cat=True):
    ...
    def decode_beam(self,prior_z, tgt, context_x, dec_state_cxz2y, lengths, ctx_cat=True):
        if ctx_cat:
...
class EncoderStateFix(nn.Module):
    ...
    def forward(self, encoder_hidden):
        ...
        if isinstance(encoder_hidden, tuple):
            ...
            h1 = self.encoder2z_layer1_h(encoder_hidden[0])
            c1 = self.encoder2z_layer1_c(encoder_hidden[1])
            ...
            h1 = self.encoder2z_layer1_h(encoder_hidden)
...
class MirrorLow(MirrorModel):
    def __init__(self, encoder_utt, encoder_ctx, decoder_cxz2y, decoder_cz2x, decoder_cyz2x, decoder_cz2y,multigpu=False, opt=None):
        super(MirrorLow, self).__init__(encoder_utt, encoder_ctx, decoder_cxz2y, decoder_cz2x, decoder_cyz2x, decoder_cz2y, multigpu, opt)
        self.encoder2z = EncoderState2zLow(opt)
    ...
    def forward(self, src, tgt, ctx, lengths, src_back, tgt_back, lengths_back, lengths_ctx, dec_state_cxz2y=None, dec_state_cyz2x=None, dec_state_cz2x=None, dec_state_cz2y=None, ctx_cat=True):
        ...
        enc_hidden_cxy = torch.cat([enc_hidden_ctx, enc_hidden_x, enc_hidden_y], -1)
        enc_hidden_cx = torch.cat([enc_hidden_ctx, enc_hidden_x], -1)
        enc_hidden_cy = torch.cat([enc_hidden_ctx, enc_hidden_y], -1)

        # (256, 2400)

        rec_z, rec_mu, rec_logvar = self.encoder2z.cxy2z(enc_hidden_cxy)
        prior_z_cx, prior_mu_cx, prior_logvar_cx = self.encoder2z.cx2z(enc_hidden_cx)
        prior_z_cy, prior_mu_cy, prior_logvar_cy = self.encoder2z.cy2z(enc_hidden_cy)
        # (256, z_dim)
        ...
        if not ctx_cat:
            ...
            # if not self.ctx_att_always:
            out_cxz2y, dec_state_cxz2y, attns_cxz2y = self.decoder_cxz2y(tgt, context_x,
                                            enc_state_cxz if dec_state_cxz2y is None
                                            else dec_state_cxz2y,
                                            context_lengths=lengths)

            out_cyz2x, dec_state_cyz2x, attns_cyz2x = self.decoder_cyz2x(tgt_back, context_y,
                                            enc_state_cyz if dec_state_cyz2x is None
                                            else dec_state_cyz2x,
                                            context_lengths=lengths_back)
        else:
            context_ctx_x = torch.cat([context_ctx, context_x], 0)
            context_ctx_y = torch.cat([context_ctx, context_y], 0)
            ...
            out_cxz2y, dec_state_cxz2y, attns_cxz2y = self.decoder_cxz2y(tgt, context_ctx_x,
                                            enc_state_cxz if dec_state_cxz2y is None
                                            else dec_state_cxz2y)

            out_cyz2x, dec_state_cyz2x, attns_cyz2x = self.decoder_cyz2x(tgt_back, context_ctx_y,
                                            enc_state_cyz if dec_state_cyz2x is None
                                            else dec_state_cyz2x,)
    
        out_cz2x, dec_state_cz2x, attns_cz2x = self.decoder_cz2x(tgt_back, context_ctx,
                                        enc_state_cz if dec_state_cz2x is None
                                        else dec_state_cz2x,
                                        context_lengths=lengths_ctx)

        out_cz2y, dec_state_cz2y, attns_cz2y = self.decoder_cz2y(tgt, context_ctx,
                                            enc_state_cz if dec_state_cz2y is None
                                            else dec_state_cz2y,
                                            context_lengths=lengths_ctx)
        kl_loss_cx2z = 0.5 * self.encoder2z.kl_loss(rec_mu, rec_logvar, prior_mu_cx, prior_logvar_cx)
        kl_loss_cy2z = 0.5 * self.encoder2z.kl_loss(rec_mu, rec_logvar, prior_mu_cx, prior_logvar_cx)
        # kl_loss_cy2z = 0.5 * self.encoder2z.kl_loss(rec_mu, rec_logvar, prior_mu_cy, prior_logvar_cy)
        results = Pack(
                        out_cxz2y=out_cxz2y, dec_state_cxz2y=dec_state_cxz2y, attns_cxz2y=attns_cxz2y,
                        out_cyz2x=out_cyz2x, dec_state_cyz2x=dec_state_cyz2x, attns_cyz2x=attns_cyz2x,
                        out_cz2x=out_cz2x, dec_state_cz2x=dec_state_cz2x, attns_cz2x=attns_cz2x,
                        out_cz2y=out_cz2y, dec_state_cz2y=dec_state_cz2y, attns_cz2y=attns_cz2y,
                        kl_loss=kl_loss_cx2z.sum() + kl_loss_cy2z.sum()
                       )
    ...
    def forward_beam(self, src, ctx, lengths, lengths_ctx, ctx_cat=True):
        ...
        # (256, 800)
        enc_hidden_cx = torch.cat([enc_hidden_ctx, enc_hidden_x], -1)
        prior_z, prior_mu, prior_logvar = self.encoder2z.cx2z(enc_hidden_cx)
        # (256, z_dim)
        enc_hidden_cxz = torch.cat([enc_hidden_ctx, enc_hidden_x, prior_z], -1)
        enc_hidden_cxz = self.decoder_init_builder.forward_single(enc_hidden_cxz)
        # enc_state_cxz = RNNDecoderState(context_x, self.hidden_size, enc_hidden_cxz)
        ...
        if ctx_cat:
            enc_state_cxz = RNNDecoderState(context_ctx_x, self.hidden_size, enc_hidden_cxz)
            return enc_state_cxz, context_ctx_x
        else:
            enc_state_cxz = RNNDecoderState(context_x, self.hidden_size, enc_hidden_cxz)
            return enc_state_cxz, context_x
...
class MirrorLow_Encshare(MirrorLow):
    def __init__(self, encoder_utt,  decoder_cxz2y, decoder_cz2x, decoder_cyz2x, decoder_cz2y,multigpu=False, opt=None):
