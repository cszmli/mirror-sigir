...
class Translator(object):
    ...
    def __init__(self, model, fields,
                 ...
                 beam_trace=False, min_length=0, use_dc=False):
        self.model = model
        self.fields = fields
        self.n_best = n_best
        ...
        import os
        embeddings_file = os.path.join("/your/folder/folder_dc_mmi/", "sif_data", "paragram_sl999_small.txt")
        weights_file = os.path.join("/your/folder/folder_dc_mmi/", "sif_data", "enwiki_vocab_min200.txt")
        pc_file = os.path.join("/your/folder/folder_dc_mmi/", "sif_data", "opensubtitles_train_data_first_component.npy")
    ...
    def translate_batch(self, batch, data):
        ...
        ctx = onmt.io.make_features(batch, 'ctx', data_type)
        ...
        dec_states, context, prior_z = self.model.forward_beam(src, ctx, src_lengths, ctx_lengths)
        prior_z = var(prior_z.repeat(beam_size, 1))
        ...
        for i in range(self.max_length):
            ...
            dec_out, dec_states, attn = self.model.decode_beam(
                prior_z, inp, context, dec_states, context_lengths)
            ...
            if not self.copy_attn:
                ...
                out = self.model.generator_cxz2y.forward(dec_out).data
                out = unbottle(out)
                # beam x tgt_vocab
            else:
                # out = self.model.generator.forward(dec_out,
                out = self.model.generator_cxz2y.forward(dec_out,
                                                   attn["copy"].squeeze(0),
                                                   src_map)
    ...
    def _run_target(self, batch, data):
        ...
        dec_out, dec_states, attn = self.model.decode_beam(
                prior_z, tgt_in, context, dec_states, src_lengths)
        ...
        for dec, tgt in zip(dec_out, batch.tgt[1:].data):
            ...
            out = self.model.generator_cxz2y.forward(dec)
            tgt = tgt.unsqueeze(1)
            scores = out.data.gather(1, tgt)
            scores.masked_fill_(tgt.eq(tgt_pad), 0)
            # print(scores.shape, gold_scores.shape)
