...
def get_fields(data_type, n_src_features, n_tgt_features, n_ctx_features):
    """
    Args:
    ...
    """
    if data_type == 'text':
        return TextDataset.get_fields(n_src_features, n_tgt_features, n_ctx_features)
    elif data_type == 'img':
...
def load_fields_from_vocab(vocab, data_type="text"):
    ...
    n_ctx_features = len(collect_features(vocab, 'ctx'))
    n_tgt_features = len(collect_features(vocab, 'tgt'))
    n_src_back_features = len(collect_features(vocab, 'src_back'))
    n_tgt_back_features = len(collect_features(vocab, 'tgt_back'))
    fields = get_fields(data_type, n_src_features, n_tgt_features, n_ctx_features)
...
def build_dataset(fields, data_type, src_path, tgt_path, ctx_path, src_dir=None,ctx_dir=None,
                  src_seq_length=0, tgt_seq_length=0,
                  src_seq_length_trunc=0, tgt_seq_length_trunc=0,
                  ctx_seq_length_trunc=0,
    ...
    ctx_examples_iter, num_ctx_feats = _make_examples_nfeats_tpl(data_type, ctx_path, ctx_dir,
                                  ctx_seq_length_trunc, sample_rate,
                                  window_size, window_stride,
                                  window, normalize_audio, 'ctx')
    ...
    if data_type == 'text':
        dataset = TextDataset(fields, 
                              ...
                              ctx_examples_iter=ctx_examples_iter,
                              num_src_feats=num_src_feats, 
                              num_tgt_feats=num_tgt_feats,
                              num_ctx_feats=num_ctx_feats,
...
def build_vocab(train_dataset_files, fields, data_type, share_vocab,
    ...
    _build_field_vocab(fields["tgt_back"], counter["tgt_back"],
                       max_size=tgt_vocab_size,
                       min_freq=tgt_words_min_frequency)
    print(" * tgt vocab size: %d." % len(fields["tgt_back"].vocab))
    ...
    if data_type == 'text':
        ...
        _build_field_vocab(fields["src_back"], counter["src_back"],
                           max_size=src_vocab_size,
                           min_freq=src_words_min_frequency)
        print(" * src vocab size: %d." % len(fields["src_back"].vocab))
        ...
        if share_vocab:
            ...
            merged_vocab = merge_vocabs(
                [fields["src"].vocab, fields["tgt"].vocab, fields["ctx"].vocab, fields["src_back"].vocab, fields["tgt_back"].vocab],
                vocab_size=src_vocab_size)
            fields["src"].vocab = merged_vocab
            fields["tgt"].vocab = merged_vocab
            fields["src_back"].vocab = merged_vocab
            fields["tgt_back"].vocab = merged_vocab
            fields["ctx"].vocab = merged_vocab
...
def _make_examples_nfeats_tpl(data_type, src_path, src_dir,
                              src_seq_length_trunc, sample_rate,
                              window_size, window_stride,
                              window, normalize_audio, side=None):
    """
