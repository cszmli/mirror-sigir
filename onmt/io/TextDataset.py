...
class TextDataset(ONMTDatasetBase):
    """ Dataset for data_type=='text'
        ...
        Args:
            ...
            use_filter_pred (bool): use a custom filter predicate to filter
                out examples?
    """
    def __init__(self, fields, src_examples_iter, tgt_examples_iter,ctx_examples_iter,src_back_examples_iter=None, tgt_back_examples_iter=None,
                 num_src_feats=0, num_tgt_feats=0,num_ctx_feats=0,
                 src_seq_length=0, tgt_seq_length=0,ctx_seq_length=0,
        ...
        self.n_ctx_feats = num_ctx_feats
        ...
        if tgt_examples_iter is not None:
            if src_back_examples_iter is not None:
                examples_iter = (self._join_dicts(src, tgt, ctx, src_back, tgt_back) for src, tgt, ctx, src_back, tgt_back in
                                zip(src_examples_iter, tgt_examples_iter, ctx_examples_iter, src_back_examples_iter, tgt_back_examples_iter))
            else:
                examples_iter = (self._join_dicts(src, tgt, ctx) for src, tgt, ctx in
                                zip(src_examples_iter, tgt_examples_iter, ctx_examples_iter))

        else:
            examples_iter =  (self._join_dicts(src, ctx) for src, ctx in
                             zip(src_examples_iter, ctx_examples_iter))
        ...
        def filter_pred(example):
            return 0 < len(example.src) <= src_seq_length \
               ...
               and 0 < len(example.ctx) <= ctx_seq_length \
               and 0 < len(example.src_back) <= src_seq_length \
               and 0 < len(example.tgt_back) <= tgt_seq_length 
    ...
    def make_text_examples_nfeats_tpl(path, truncate, side):
        ...
        """
        assert side in ['src', 'tgt', 'ctx', 'src_back', 'tgt_back']
    ...
    def get_fields(n_src_features, n_tgt_features, n_ctx_features=None):
        ...
        fields["src_back"] = torchtext.data.Field(
            pad_token=PAD_WORD,
            include_lengths=True)
        ...
        fields["tgt_back"] = torchtext.data.Field(
            init_token=BOS_WORD, eos_token=EOS_WORD,
            pad_token=PAD_WORD)
        ...
        fields["src_back_map"] = torchtext.data.Field(
            use_vocab=False, 
            postprocessing=make_src, sequential=False)
        
    
        fields["ctx_map"] = torchtext.data.Field(
            use_vocab=False, 
            postprocessing=make_src, sequential=False)
        ...
        fields["alignment_back"] = torchtext.data.Field(
            use_vocab=False, 
            postprocessing=make_tgt, sequential=False)
    ...
    def _dynamic_dict(self, examples_iter):
        for example in examples_iter:
            ...
            ctx = example["ctx"]
            ctx_vocab = torchtext.vocab.Vocab(Counter(ctx))
            self.src_vocabs.append(ctx_vocab)
            # Mapping source tokens to indices in the dynamic dict.
            ctx_map = torch.LongTensor([ctx_vocab.stoi[w] for w in ctx])
            example["ctx_map"] = ctx_map
            
            if "src_back" in example:
                src_back = example["src_back"]
                src_back_vocab = torchtext.vocab.Vocab(Counter(src_back))
                self.src_vocabs.append(src_back_vocab)
                # Mapping source tokens to indices in the dynamic dict.
                src_back_map = torch.LongTensor([src_back_vocab.stoi[w] for w in src_back])
                example['src_back_map'] = src_back_map
            ...
            if "tgt_back" in example:
                tgt = example["tgt_back"]
                mask = torch.LongTensor(
                        [0] + [src_vocab.stoi[w] for w in tgt] + [0])
                example["alignment_back"] = mask
